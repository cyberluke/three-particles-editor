...
var createDefaultMeshTexture = () => {
  try {
    ...
    if (context) {
      ...
      const texture8 = new THREE3.CanvasTexture(canvas);
...
var writeFluidDomainUniforms = (uniforms, fluid) => {
  if (!fluid) return;
  const setV4 = (slot, v) => {
    const target = slot?.value;
    if (!target || typeof target.set !== 'function') return;
    target.set(Number(v[0]) || 0, Number(v[1]) || 0, Number(v[2]) || 0, Number(v[3]) || 0);
  };
...
var createParticleSystem = (config = DEFAULT_PARTICLE_SYSTEM_CONFIG, externalNow) => {
  ...
  if (rrType === 'FLUID' /* FLUID */) {
    ...
    pipeline.uniforms.fluidSphereDomain = solver.uniforms.sphereDomain;
    pipeline.uniforms.fluidPointerPos = solver.uniforms.pointerPos;
    pipeline.uniforms.fluidPointerVel = solver.uniforms.pointerVel;
    writeFluidDomainUniforms(pipeline.uniforms, normalizedConfig.renderer.fluid);
    const fl = pipeline;
    fl.computeNodes = [
      ...(fl.computeNodes ??
    ...
    fluidSolverPassNames = fl.passNames;
    fl.passLayouts = [
      ...(fl.passLayouts ?? []),
  ...
  if (rrType === 'FLUID' /* FLUID */) {
    fluidScreenSpacePasses = material.__fluidPassNodes?.length ?? 0;
  }
  ...
  const particleSystem = trailGeometry
    ...
    : useInstancing
      ? new THREE3.Mesh(fluidPassGeometry ?? geometry, material)
  ...
  return {
    ...
    bindCamera: (cam) => {
      if (!cam) return;
      const mats = [material, trailMaterial];
      for (const m of mats) {
      ...
      fluidSolverId
        ? {
            ...
            boxWidthRatio: Number(
              pipeline.uniforms.fluidBoxWidthRatio?.value ?? fluidBoxWidthRatioValue
            ),
...
var updateParticleSystemInstance = (props, { now, delta, elapsed }) => {
  ...
  if (generalData.fluidSolver) {
    ...
    if (u.fluidBoxWidthRatio) {
      ...
      u.fluidBoxWidthRatio.value =
        (typeof liveRatio === 'number' && Number.isFinite(liveRatio)
          ? liveRatio
          : generalData.fluidBoxWidthRatio) ?? 1;
    }
    writeFluidDomainUniforms(u, normalizedConfig.renderer.fluid);
  }
      ...
      if (useTwistPrevention && prevNormal && finalCount >= 2) {
        ...
        if (tLen > 1e-4) {
          ...
          let upx = 0,
            upy = 1,
            upz = 0;
          const dot10 = ntx * upx + nty * upy + ntz * upz;
          if (Math.abs(dot10) > 0.999) {
            upx = 1;
            upy = 0;
            upz = 0;
          }
...
//# sourceMappingURL=chunk-FK3Z4JZ7.js.map
//# sourceMappingURL=chunk-FK3Z4JZ7.js.map
