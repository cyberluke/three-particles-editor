...
| 54  | electric-arc                 | standalone kind (off-canvas)                          | PASS (VISUAL) |
| 55  | fluid-metaball               | FLUID renderer, screen-space pass chain               | PASS (VISUAL) |
| 56  | mls-mpm-dambreak             | FLUID + MLS-MPM solver (ref. `mls-mpm/`)              | PASS          |
| 57  | sph-dambreak                 | FLUID + SPH solver (ref. `sph/`)                      | PASS          |
| 58  | mls-mpm-sphere-debug         | FLUID + `sphereRender:true` (ref. `sphere.wgsl`)      | PASS (VISUAL) |
| 59  | mls-mpm-large-dambreak       | FLUID + MLS-MPM big lattice, box `[45,40,80]`         | PASS (VISUAL) |
| 60  | sph-large-dambreak           | FLUID + SPH big lattice, halfBox `[1.2,2,1.2]` on 62³ | PASS (VISUAL) |
