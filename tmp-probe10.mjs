...
import { context, Fn, If, Discard, attribute, uniform, vec2, vec3, vec4, float, texture, abs, length, min, max, floor, mod, round, smoothstep, cos, sin, screenUV, varyingProperty, getCurrentStack, modelViewMatrix, positionLocal, cameraProjectionMatrix } from "three/tsl";
...
const uViewportHeight = uniform(float(720));
...
const vertex = Fn(() => {
  ...
  If(aColor.w.greaterThan(0.0), () => {
    ...
    const mvPosition = modelViewMatrix.mul(vec4(aInstanceOffset.xyz, 1.0)).toVar();
    const dist = length(mvPosition.xyz);
    const pointSizePx = aParticleState.y.mul(100.0).div(dist);
...
let nErr = 0; const oe = console.error; const msgs=[]; console.error = (...a)=>{ nErr++; msgs.push(String(a[0]).split("\n")[0]); };
...
b.build();
console.error = oe;
oe("errors:", nErr, "v=", String(b.vertexShader||"").length, "f=", String(b.fragmentShader||"").length);
msgs.slice(0,4).forEach(m=>oe("  ",m));